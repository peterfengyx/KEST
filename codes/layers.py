...
class GPT2VAEOutputWithPast(ModelOutput):
    ...
    nag_loss: Optional[torch.FloatTensor] = None
    nag_ag_loss: Optional[torch.FloatTensor] = None
    ...
    nag_logits: torch.FloatTensor = None
    nag_ag_logits: Optional[torch.FloatTensor] = None
...
class LossWrapper(object):
    def __init__(self, pad_idx, kl_lambda=0, label_smoothing_factor=0.0):
        ...
        #self._criterion_nagag = nn.CrossEntropyLoss(ignore_index=pad_idx)
        ...
        self._msecriterion=nn.MSELoss()
        self._cossim=nn.MSELoss()#CosineEmbeddingLoss()
        self._mmd=MMD_loss()
    ...
    def genCELoss_nag(self, logits, tgts, attn_mask, type_ids, pretrain=False):
        # [CLS] A [SEP] B [SEP]
        # logits (B, L, V), tgts (B, L), type_ids (B, L)
        ...
        shift_labels = tgts.contiguous()
        shift_type_ids = type_ids.contiguous()
    ...
    def genCELoss_nagag(self, logits, tgts, attn_mask, type_ids, pretrain=False):
        # [CLS] A [SEP] B [SEP]
        # logits (B, L, V), tgts (B, L), type_ids (B, L)
        ...
        shift_labels = tgts[..., 2:].contiguous()
        shift_type_ids = type_ids[..., 2:].contiguous()
    ...
    def mmdLoss(self,prob, prob_t, attn_mask, type_ids):
        #TODO: temp code
        bsz = prob.shape[0]
        ...
        flatten_logits = shift_logits.view(bsz, -1)
        flatten_labels = shift_labels.view(bsz, -1)
        loss=self._mmd(flatten_logits,flatten_labels)
        return loss
    ...
    def emb_dist(self,emb,emb_t,attn_mask,type_ids):
        shift_logits = emb[..., :-1, :].contiguous()
        shift_labels = emb_t[..., 1:].contiguous()
        shift_type_ids = type_ids[..., 1:].contiguous()
        ...
        flatten_logits = shift_logits.view(-1, shift_logits.size(-1))
        flatten_labels = shift_labels.view(-1, shift_labels.size(-1))
        target=torch.ones(flatten_logits.shape[0]).to(shift_type_ids)
        loss = self._cossim(flatten_logits, flatten_labels)#,target)
        return loss
    
    def emb_dist_nagag(self,emb,emb_t,attn_mask,type_ids):
        shift_logits = emb[..., :-2, :].contiguous()
        shift_labels = emb_t[..., 2:].contiguous()
        shift_type_ids = type_ids[..., 2:].contiguous()
