...
def load_plm(model, plm_dir):
    ...
    map_dic2 = map_dic
    for k,v in map_dic2.items():
        if "transformers" in v:
            new_v = copy.deepcopy(v).replace("transformers", "ag_decoder")
            map_dic2[k]=new_v
    sub_dic = {}
    ...
    for k, v in plm_ckpt.items():
        ...
        try:
            if k in map_dic2:
                sub_dic[map_dic2[k]] = v
        except:
            print("ERROR: did not find ag_decoder params")
            pass
...
def print_samples(tokenizer, gen_logits, tgts, type_ids,
    ...
    print("\n")
    info_str = "Training gen loss: %.2f, cl loss: %.2f, " + \
        "\n ppl: %.2f, accu: %.2f, f1: %.2f" + \
        "\n lr: %.2f 1e-4, gen kl weight: %.4f, cl kl weight: %.4f \n"
    
    loss_info = info_str % (gen_loss, cl_loss, np.exp(gen_loss), accu*100, f1*100,
            lr * 1e4, gen_kl_weight, cl_kl_weight
    )
...
def getAccuF1(logits, labels):
    ...
    f1 = f1_score(cl_labels, cl_preds, zero_division=1, average='macro')
