...
class NLGTrainer(Seq2SeqTrainer):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if self.state.global_step>=extra_args.st_start_step and self.state.global_step%self.train_cycle==extra_args.st_start_step%self.train_cycle:
                    ...
                    if extra_args.use_prior==False:
                        ...
                        idx=list(range(len(self.train_loader2)+self.self_training_steps_cls))
                        random.shuffle(idx)
                        idx=idx[:self.self_training_steps_gen]
                        for u_step, u_inputs in enumerate(self.train_loader2):
                            if u_step in idx:
                                u_inputs=self.preprocess(u_inputs)
                                pseudo_inputs=self.get_soft_label(model, inputs=u_inputs, 
                                                              return_emb=extra_args.return_emb,
                                                              temperature=extra_args.gen_temperature,
                                                              mode=extra_args.st_mode)
                                pseudo_list.append(pseudo_inputs)
                        print(len(pseudo_list)*self.args.train_batch_size," Soft label (from labeled) generated.")
                        ...
                        for i in range(self.gen_num_batch):
                            pseudo_inputs=self.gen_pseudo_data_from_label(model, num_labels=self.model.config.n_label, 
                    ...
                    print("Pseudo Data Generation time",t_end-t_begin)
                    t_begin=time.perf_counter()
                    if unlabelled_dataset is not None:
                        if extra_args.st_sampling in ["topk", "sample"]:
                            ...
                            for u_step, u_inputs in enumerate(unlabelled_dataloader):
                                ...
                                if extra_args.use_prior==False:
                                    if u_step+len(self.train_loader2) in idx:
                                        pseudo_l_inputs=self.preprocess(pseudo_l_inputs)
                                        pseudo_inputs=self.get_soft_label(model, inputs=pseudo_l_inputs, 
                                                              return_emb=extra_args.return_emb,
                                                              temperature=extra_args.gen_temperature,
                                                              mode=extra_args.st_mode)
                                        pseudo_list.append(pseudo_inputs)
                    ...
                    t_end=time.perf_counter()
                    print("Pseudo Label Annotation time",t_end-t_begin)
                    print(len(pseudo_list)*self.args.train_batch_size," Pseudo Data Generated.")
                    model.train()
                    for i in range(self.gen_num_batch):
                        pseudo_inputs=pseudo_list[i]
                        pseudo_inputs=self.preprocess(pseudo_inputs)
    ...
    def get_soft_label(
        ...
        with torch.no_grad():
            ...
            if mode=='ag':
                ...
                logit=outs['nag_logits'].detach()
            
            
            beginer = torch.tensor([bos_idx]).to(inp_dic['seq_attn_mask'])
            mask_position = inp_dic['masked_token_type_ids']*mask_idx
            
            if extra_args.soft_label==True:
                if return_emb:
                    beginer = model.transformer.wte(beginer).tile(outs['gen_logits'].shape[0],1,1)
                    raw_mask=inp_dic['masked_token_type_ids'].unsqueeze(-1).tile(beginer.shape[-1])
                    oppo_mask= (~inp_dic['masked_token_type_ids']).unsqueeze(-1).tile(beginer.shape[-1])
                    inp_prob=model.logit2prob(logit,temp=extra_args.gen_temperature)
                    inp_emb=torch.matmul(inp_prob,model.transformer.wte.weight)
                    
                    #add mask
                    mask_emb=model.transformer.wte(mask_position).masked_fill(oppo_mask,0.0)
                    temp_emb=torch.cat([beginer,inp_emb[:,:-1,:]], dim=1).masked_fill(raw_mask,0.0)
                    outp_dic['past_emb']=torch.cat([beginer,inp_emb[:,:-1,:]], dim=1)
                    outp_dic['past_emb_nag']=mask_emb+temp_emb
                else:#TODO
                ...
                if return_emb:
                    probs = model.logit2prob(logit,temp=extra_args.gen_temperature).reshape(-1,logit.shape[-1])
                    sample_out = torch.multinomial(probs, num_samples=1, replacement=True).reshape(outs['gen_logits'].shape[0],-1)
                    
                    beginer = torch.tensor([bos_idx]).tile(outs['gen_logits'].shape[0],1).to(inp_dic['seq_attn_mask'])
                    original_out=torch.cat([beginer,sample_out[:,:-1]],dim=1)
                    masked_out= original_out.masked_fill(inp_dic['masked_token_type_ids'],mask_idx)
                    outp_dic['past_emb']= model.transformer.wte(original_out)
                    outp_dic['past_emb_nag']= model.transformer.wte(masked_out)
                else:
                    beginer = F.one_hot(beginer, num_classes=logit.shape[-1]).tile(outs['gen_logits'].shape[0],1,1)
                    outp_dic['past_logit']=torch.cat([beginer,logit[:,:-1,:]], dim=1)
    ...
    def preprocess(self,inputs):
        prob=extra_args.mask_prob
        prob_tensor=torch.tensor([1-prob, prob])
        mask_idx = self.tokenizer.convert_tokens_to_ids(self.tokenizer.mask_token)
        new_input=inputs
        
        unk_mask=torch.multinomial(prob_tensor,inputs['seq_ids'].shape[0]*inputs['seq_ids'].shape[1],replacement=True)
        unk_mask=unk_mask.reshape(inputs['seq_ids'].shape).to(inputs['seq_ids'])
        unk_mask[:,:4]=torch.zeros([inputs['seq_ids'].shape[0],4]).to(inputs['seq_ids'])
        new_input['masked_token_type_ids']=(unk_mask*inputs['seq_attn_mask']).bool()
        new_input['masked_seq_ids']=inputs['seq_ids'].masked_fill(new_input['masked_token_type_ids'],mask_idx)
    ...
    def gen_pseudo_data_from_label(
        ...
        max_len_gen=0
        min_len = extra_args.min_gen_len
        max_len = extra_args.max_gen_len
        ...
        if mode=='ag':
            ...
            pseudo_label=torch.zeros((num_per_class),dtype=torch.long, device=model.device)
            for ii in range(1,num_labels):
                pseudo_label=torch.cat([pseudo_label,torch.ones((num_per_class),dtype=torch.long, device=model.device)*ii],dim=0)
            ...
            with torch.no_grad():
                outs = generator.generate(
                        ...
                        use_cache=True, max_length=max_len, min_len=min_len,
            ...
            if prompt:
                temp_id=[mask_idx]*max_len
                temp_id[:len(prompt["body_ids"])]=prompt["body_ids"]
                temp_att=[1]*max_len
                temp_att[:len(prompt["attention_mask"])]=prompt["attention_mask"]
                temp_id[random.randint(1,4)]=mask_idx
            else: #nag,nag-ag
                temp_id=[mask_idx]*max_len
                temp_id[0]=bos_idx
                temp_att=[1]*max_len
                
            beginer=torch.tensor(temp_id,dtype=torch.long, device=model.device).tile(num_per_class*num_labels,1)
            att=torch.tensor(temp_att,dtype=torch.long, device=model.device).tile(num_per_class*num_labels,1)
            token_type=att
            pseudo_label=torch.zeros((num_per_class),dtype=torch.long, device=model.device)
            for ii in range(1,num_labels):
                pseudo_label=torch.cat([pseudo_label,torch.ones((num_per_class),dtype=torch.long, device=model.device)*ii],dim=0)
            ...
            with torch.no_grad():
                y_h=model.label_embed(pseudo_label)
                nag_logits = model.get_gen_outputs(
                    beginer, att,
                    token_type, y_h, causal_mask=False
                )
                probs = nn.functional.softmax(nag_logits, dim=-1).reshape(-1,nag_logits.shape[-1])
                outs = torch.multinomial(probs, num_samples=1, replacement=True).reshape(num_per_class*num_labels,-1)
                if mode=='nag-ag':
                    beginer=outs
                    att=torch.ones(outs)
                    token_type=att
                    nag_logits = model.get_gen_outputs(
                        beginer, att,
                        token_type, y_h, causal_mask=True
                    )#.squeeze(0)
                    probs = nn.functional.softmax(nag_logits, dim=-1).reshape(-1,nag_logits.shape[-1])
                    outs = torch.multinomial(probs, num_samples=1, replacement=True).reshape(num_per_class,-1)
            ...
            for i in range(num_per_class*num_labels):
                ...
                #print(len(seq))
                if len(seq)>max_len_gen:
                    max_len_gen=min(max_len,len(seq))
                titles.append(torch.tensor(seq,dtype=torch.long, device=model.device))
                    
                    
        outp_dic['title_ids']=torch.zeros((len(titles),max_len_gen),dtype=torch.long, device=model.device)
        outp_dic['body_ids']=torch.zeros((len(titles),max_len_gen),dtype=torch.long, device=model.device)
        ...
        for i in range(len(titles)):
            ...
            outp_dic['body_ids'][i,:5]=outp_dic['title_ids'][i][:5]
        ...
        outp_dic['body_attn_mask']=outp_dic['seq_attn_mask']
        outp_dic['body_token_type_ids']=outp_dic['seq_token_type_ids']
        if return_logit:
            if mode=='ag':
            ...
            if extra_args.return_emb:
                with torch.no_grad():
                    inp_prob=model.logit2prob(torch.cat([beginer,logit[:,:(max_len_gen-1),:]], dim=1),temp=extra_args.gen_temperature)
                    outp_dic['past_emb']=torch.matmul(inp_prob,model.transformer.wte.weight)
            else:
                outp_dic['past_logit']=torch.cat([beginer,logit[:,:(max_len_gen-1),:]], dim=1)
        else:
            if extra_args.return_emb:
                with torch.no_grad():
                    outp_dic['past_emb']=model.transformer.wte.outp_dic['seq_ids']
    ...
    def compute_loss(self, model, inputs, valid=False):   
        ...
        total_loss = outs['gen_loss']*extra_args.gen_weight + outs['cl_loss'] * extra_args.cl_weight + \
            outs['nag_loss']*extra_args.nag_weight+outs['nag_ag_loss']*extra_args.nag_ag_weight

        accu, f1 = utils.getAccuF1(outs['cl_logits'], inputs['cl_labels'])
        
        loss_dic = {'gen_loss':outs['gen_loss'], 'cl_loss': outs['cl_loss'],
                    'nag_loss': outs['nag_loss'], 'nag_ag_loss': outs['nag_ag_loss'],
